package com.dave.microservices.alpha.auth.config;
...
import com.dave.microservices.alpha.auth.jose.Jwks;
...
public class AuthorizationServerConfigs {
	
	private static final Logger LOG = LoggerFactory.getLogger(AuthorizationServerConfigs.class);
	...
	@Bean
	public JwtDecoder jwtDecorder(JWKSource<SecurityContext> jwkSource) {
	...
	private Consumer<List<AuthenticationProvider>> configureAuthenticationValidator() {
		...
		return (authenticationProviders) -> {
			authenticationProviders.forEach(authProvider -> {
				if (authProvider instanceof OAuth2AuthorizationCodeRequestAuthenticationProvider) {
					...
					((OAuth2AuthorizationCodeRequestAuthenticationProvider)authProvider).setAuthenticationValidator(authenticationValidator);
				}
			});
		};
	}
	...
	static class CustomRedirectUriValidator implements Consumer<OAuth2AuthorizationCodeRequestAuthenticationContext> {
		...
		public void accept(OAuth2AuthorizationCodeRequestAuthenticationContext authenticationContext) {
			OAuth2AuthorizationCodeRequestAuthenticationToken requestAuthentication = authenticationContext.getAuthentication();
			
			RegisteredClient registeredClient = authenticationContext.getRegisteredClient();
			
			String requestedRedirectUri = requestAuthentication.getRedirectUri();
